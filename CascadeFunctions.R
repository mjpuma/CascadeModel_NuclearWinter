...
sim_1c <- function(c_init, a_init, P0, R0, E0, cfrac = 0, asym = TRUE,
  ...
  nc <- length(P0)
  if (a_init < -1 || a_init > 1) stop("a_init must be between -1 and 1")
...
sim_mc <- function(a_init, P0, R0, E0, cfrac, asym, kmax, amin, anim_out) {
    ...
    nc <- length(P0)
    if (any(a_init < -1 || a_init > 1)) stop("a_init values must be between -1 and 1")
...
# Get a few summary statistics by country from the output of get_stats_allc function
...
get_stats_multiyr <- function(result_multiRcoef) {
  # Initial net supply by country
  S0 <- result_multiRcoef$`2005`$P0 + colSums(result_multiRcoef$`2005`$E0) - rowSums(result_multiRcoef$`2005`$E0)
  # Magnitude of initial shock by sim
  nsim <- ncol(result_multiRcoef$`2005`$dR)
  # this returns a vector of length nsim where each element is the 
  # total dP (summed over initially affected countries) in each sim.
