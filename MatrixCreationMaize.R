...
#filter for maize and maize products 
trade_dat<-trade_dat[(trade_dat$cropid==56 | 
                        trade_dat$cropid==57 |
                        trade_dat$cropid==58 |
                        trade_dat$cropid==60|
                        trade_dat$cropid== 446),]
...
FAO165<-data_frame(FAO=dimnames(E0)[[1]])
FAO165<-merge(FAO165, ciso,by.x='FAO',by.y='FAO', all.x=TRUE)
FAO165<-FAO165[,(1:3)]
FAO165$FAO<-as.numeric(FAO165$FAO)
FAO165<-FAO165[order(FAO165$FAO),]
write.csv(FAO165, "inputs/Countries162.csv", row.names = FALSE)

dimnames(E0) <- list(FAO165$iso3, FAO165$iso3) 

save(E0, file="inputs/MaizeAllCommodities2007E0.RData")

##to export matrices with country names
Tkbyc<-as.data.frame(E0, row.names = dimnames(E0)[[1]], col.names = dimnames(E0)[[2]] )
write.csv(Tkbyc, "MaizeTrade2007.csv")
...
P0 <- P0[-no_dat]
names(P0)<-FAO165$iso3


save(P0, E0, file="inputs/Maize2007P0E0.RData")
save(P0, file= "inputs/Maize2007P0.Rdata")

##to export matrix with country names
Pkbyc<-data_frame(iso3=names(P0), P0=P0)
write.csv(Pkbyc,"MaizeProduction2007.csv", row.names=FALSE)
...
#keep only Corn
psd0608<-psd0608[(psd0608$Product=="Corn"),]
...
conv_kcal<-conv_kcal[(conv_kcal$cropname=="Corn"),]
...
save(R0, file = "inputs/Maize2007R0.RData")


Rkbyc<-data_frame(iso3=names(R0), R0=R0)
write.csv(Rkbyc,"MaizeReserves2007.csv", row.names=FALSE)
