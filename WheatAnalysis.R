...
load("Wheat2007P0.RData")
load("Wheat2007R0.RData")
load("WheatAllCommodities2007E0.RData")


# Anomaly data
Anomalies<-read.csv("wheat_production_change_country_yr1-15_multi_model_mean_2019-08-14.csv")
...
WheatResultsY1 <- sim_mc_multi(Y1Dec, P0, R1, E0, cfrac = 0.1,
...
R3<-((WheatResultsY2$R0 + WheatResultsY2$dR)+(WheatResultsY2$P0*Reserves$Y3Inc))
R3<-R3[,1]
...
WheatResultsY3 <- sim_mc_multi(Y3Dec, P0, R3, E0, cfrac = 0.1,
                               asym = T, kmax = 1000, amin = 1E-5)
...
R4<-((WheatResultsY3$R0 + WheatResultsY3$dR)+(WheatResultsY3$P0*Reserves$Y4Inc))
R4<-R4[,1]
...
WheatResultsY4 <- sim_mc_multi(Y4Dec, P0, R4, E0, cfrac = 0.1,
                               asym = T, kmax = 1000, amin = 1E-5)
...
R5<-((WheatResultsY4$R0 + WheatResultsY4$dR)+(WheatResultsY4$P0*Reserves$Y5Inc))
R5<-R5[,1]
...
WheatResultsY5 <- sim_mc_multi(Y5Dec, P0, R5, E0, cfrac = 0.1,
                               asym = T, kmax = 1000, amin = 1E-5)
...
Results<-data.frame(iso3=names(WheatResultsY1$P0),
                    R1 = R1, 
                    dRY = WheatResultsY1$dR, 
                    R2=R2,
                    dRY2 = WheatResultsY2$dR, 
                    R3=R3,
                    dRY3= WheatResultsY3$dR, 
                    R4=R4,
                    dRY4= WheatResultsY4$dR, 
                    R5=R5,
                    dRY5v1 = WheatResultsY5$dR)

write.csv(Results, "Wheat_5yr_NuclearWinter.csv")
